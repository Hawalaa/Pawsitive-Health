...
export default function UpdateFeedingRecordModal({ isOpen, onClose, selectedPetId, feeding_id }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await updateFeedingMutation({ updatedFeeding, petId: selectedPetId, feeding_id: feeding_id }).unwrap();
            console.log(feeding_id)
    ...
    return (
        <Modal open={isOpen} onClose={onClose}>
            <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 400, bgcolor: 'background.paper', border: '2px solid #000', boxShadow: 24, p: 4 }}>
                ...
                <TextField value={foodType} onChange={handleFoodTypeChange} fullWidth />
                <InputLabel>Amount(Cup)</InputLabel>
                <TextField type="number" value={amount} onChange={handleAmountChange} fullWidth />
